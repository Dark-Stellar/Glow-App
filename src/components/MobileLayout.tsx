...
export function MobileLayout({
  ...
  return <div className="min-h-screen bg-background flex flex-col">
      <header className="sticky top-0 z-50 w-full border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
        <div className="container h-12 px-[6px] mx-0 flex-row flex items-center justify-between gap-[250px]">
      ...
      <main className="flex-1 pb-16 overflow-auto px-0 mx-0">
        {children}
      </main>
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-card border-t border-border z-40">
        <div className="flex justify-around items-center h-14 max-w-full overflow-x-auto">
          ...
          return <Link key={path} to={path} className={cn("flex flex-col items-center justify-center flex-1 min-w-[56px] h-full gap-0.5 transition-colors", isActive ? "text-primary" : "text-muted-foreground")}>
