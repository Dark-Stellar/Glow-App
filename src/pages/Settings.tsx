...
const Settings = () => {
  ...
  const savePreferences = useCallback(async () => {
    ...
    if (error) toast.error("Failed to save preferences");else {
  ...
  const handleSendTestNotification = useCallback(async () => {
    ...
    try {
      ...
      if (result.success) toast.success("Test notification sent!");else toast.error(result.error || "Failed to send test notification");
    } catch (error) {
  ...
  const handleExportAll = useCallback(async () => {
    ...
    try {
      ...
      doc.setFontSize(22);
      doc.text("Glow - Productivity Report", pageWidth / 2, 18, {
        align: 'center'
      });
      doc.setFontSize(10);
      doc.text("Measure. Grow. Glow.", pageWidth / 2, 28, {
        align: 'center'
      });
      ...
      for (let i = 0; i < reports.length; i++) {
        ...
        if (daysDiff === i && reports[i].productivityPercent >= 60) currentStreak++;else break;
      }
      ...
      statBoxes.forEach((stat, idx) => {
        ...
        doc.setTextColor(255, 255, 255);
        doc.setFontSize(14);
        doc.text(stat.value, x + boxWidth / 2, yPos + 10, {
          align: 'center'
        });
        doc.setFontSize(7);
        doc.text(stat.label, x + boxWidth / 2, yPos + 17, {
          align: 'center'
        });
      });
      doc.setTextColor(0, 0, 0);
      ...
      if (allTaskTitles.size > 0) {
        ...
        doc.setTextColor(255, 255, 255);
        doc.setFontSize(16);
        doc.text("Task Performance Summary", pageWidth / 2, 15, {
          align: 'center'
        });
        doc.setTextColor(0, 0, 0);
      ...
      <div className="container max-w-2xl mx-auto p-4 space-y-4">
        ...
        <Card className="p-4">
          ...
          <div className="space-y-4">
            {!browserNotificationsGranted && <div className="p-3 bg-muted rounded-lg">
            ...
            {browserNotificationsGranted && <div className="p-3 bg-success/10 rounded-lg text-sm text-success flex items-center gap-2">
            ...
            <div className="space-y-3 pt-2 border-t">
              ...
              {quietHoursEnabled && <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-2">
